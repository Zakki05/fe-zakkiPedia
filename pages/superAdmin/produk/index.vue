...
const removeData = async (id) => {
    const res = await $fetch(`http://be-zakkiPedia.test/api/v1/produk/${id}`, {
        headers: header,
        method: "DELETE"
    })
    toast.add({ title: 'Berhasil Hapus Data Produk' })
                ...
                <div class="text-center py-6 bg-[#f9f5ee] border border-[#e0d6c3] rounded-md shadow-md mx-auto my-8">
                    <div class="flex flex-col items-center my-3 mx-5">
                        ...
                        <div
                            ...
                            <NuxtLink to="/superAdmin/produk/create">
                                <button
                                    class="inline-flex justify-center items-center px-6 lg:px-10 py-2 bg-[#c8a876] hover:bg-[#b89b6c] text-white text-sm font-medium rounded-md shadow transition duration-300">
                                    + Tambah Produk
                                </button>
            ...
            <div v-else class="shadow-lg sm:rounded-lg pl-4 pr-4 pt-2 pb-4 mt-4 bg-[#f9f5ee] border border-[#e0d6c3]">
                <div class="flex flex-col sm:flex-row flex-wrap sm:space-y-0 items-center justify-between py-2 px-2">
                    ...
                    <div class="w-full sm:w-auto flex justify-center sm:justify-end mt-3 sm:mt-0">
                        <NuxtLink to="/superAdmin/produk/create" class="w-full">
                            <button type="button"
                    ...
                    <div v-for="(produk, index) in sortedProduks" :key="index"
                        ...
                        <div class="bg-[#f8f2e7] p-4 mb-4 rounded-lg shadow-inner">
                            ...
                            <div class="flex space-x-3 justify-center">
                                <NuxtLink :to="`/superAdmin/produk/createProdukDetail-${produk.id}`">
                                    <button type="button"
                                        ...
                                        <UIcon name="i-heroicons-plus-circle-16-solid" class="w-4 h-4" />
                                    </button>
                                </NuxtLink>
                                <button type="button" @click="deleteModal(produk.id)"
                                    class="text-white bg-red-500 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-full text-sm p-1.5 text-center inline-flex items-center">
                                    <UIcon name="i-heroicons-trash-solid" class="w-4 h-4" />
                                </button>
                            </div>
                        </div>
                        <!-- Delete -->
                        <div :id="`modalDelete-${produk.id}`" tabindex="-1"
                            class="hidden bg-black bg-opacity-20 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full md:inset-0 h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow">
                                    <button type="button" @click="hideModal(produk.id)"
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500">Apakah Kamu Ingin Menghapus
                                            {{
                                                produk.nama_produk }}?
                                        </h3>
                                        <button type="button" @click="removeData(produk.id)"
                                            class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                            Yaa, Hapus Data
                                        </button>
                                        <button type="button" @click="hideModal(produk.id)"
                                            class="py-2.5 px-5 mt-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">Tidak,
                                            Batalkan</button>
                                    </div>
                                </div>
                            </div>
