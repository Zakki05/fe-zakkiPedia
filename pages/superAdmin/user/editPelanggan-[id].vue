...
useSeoMeta({
    ...
    description: 'This is Edit Pelanggan',
})
...
if (user.value) {
    ...
    formData.value.nama_panggilan = user.value.nama_panggilan
    formData.value.nama_lengkap = user.value.nama_lengkap
    formData.value.telepon = user.value.telepon
} else {
    ...
    const res = await $fetch(`http://be-zakkipedia.test/api/v1/user/${route.params.id}`,
        {
            ...
            body: {
                ...
                nama_panggilan: formData.value.nama_panggilan,
                nama_lengkap: formData.value.nama_lengkap,
                telepon: formData.value.telepon,
                alamatPelanggan: formattedAlamat.value,
            },
        }
    );
    console.log(res)

    toast.add({ title: 'Berhasil Edit Data User dan Pelanggan' })
        ...
        <div v-else
            ...
            <form class="space-y-6" @submit.prevent="update">
                <div class="flex flex-row items-center">
                    ...
                    <h5 class="text-2xl font-medium text-gray-700">Edit Data Pelanggan</h5>
                </div>
