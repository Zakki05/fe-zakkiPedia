...
async function save() {
    ...
    formStore.append('nama', formData.value.nama)
    formStore.append('password', formData.value.password)
    formStore.append('alamat', formattedAlamat.value)
    console.log(formData)

    const res = await $fetch('http://be-zakkipedia.test/api/v1/user', {
        method: 'POST',
        headers: {
    ...
    toast.add({ title: 'Success Menambahkan Data User' })
        ...
        <div v-else class="w-full p-5 mt-8 bg-white border border-gray-200 rounded-lg shadow-md sm:p-6 md:p-8 pb-4 px-2 py-5">
            <form class="space-y-6" @submit.prevent="save">
                <div class="flex flex-row items-center">
                    ...
                    <h5 class="text-2xl font-medium text-gray-700">Buat Admin Baru</h5>
                </div>
