...
const formattedAlamat = computed(() => {
    ...
    return `${formData.value.alamatPelanggan}, ${villageName}, ${subDistrictName}, ${cityName}, ${provinceName}`;
});
...
async function save() {
    ...
    if (!formData.value.nama_panggilan) {
    ...
    if (!formData.value.provinces) {
        errors.provinces = 'Provinsi harus dipilih';
        hasError = true;
    } else {
        errors.provinces = '';
    }

    if (!formData.value.citys) {
        errors.citys = 'Kota harus dipilih';
        hasError = true;
    } else {
        errors.citys = '';
    }

    if (!formData.value.subDistricts) {
        errors.subDistricts = 'Kecamatan harus dipilih';
        hasError = true;
    } else {
        errors.subDistricts = '';
    }

    if (!formData.value.villages) {
        errors.villages = 'Kelurahan harus dipilih';
        hasError = true;
    } else {
        errors.villages = '';
    }

    if (!formData.value.alamatPelanggan) {
        errors.alamatPelanggan = 'Jalan harus diisi';
        hasError = true;
    } else {
        errors.alamatPelanggan = '';
    }

    if (!formData.value.telepon) {
    ...
    formStore.append('role_id', formData.value.role_id)
    formStore.append('email', formData.value.email)
    formStore.append('username', formData.value.username)
    formStore.append('password', formData.value.password)
    console.log(formData)
    formStore.append('nama_lengkap', formData.value.nama_lengkap);
    formStore.append('nama_panggilan', formData.value.nama_panggilan);
    formStore.append('alamatPelanggan', formattedAlamat.value);
    formStore.append('telepon', formData.value.telepon);

    const res = await $fetch('http://be-zakkipedia.test/api/v1/storePelanggan', {
        method: 'POST',
        headers: {
    ...
    toast.add({ title: 'Success Menambahkan Data Pelanggan' })
    ...
    formData.value.alamatPelanggan = ''
    formData.value.nama_panggilan = ''
    formData.value.nama_lengkap = ''
    formData.value.telepon = ''
        ...
        <div v-else
            ...
            <form class="space-y-6" @submit.prevent="save">
                <div class="flex flex-row items-center">
                    ...
                    <h5 class="text-2xl font-medium text-gray-700">Buat Pelanggan Baru</h5>
                </div>
                ...
                <div class="grid gap-4 mb-4 grid-cols-2 mt-7 px-1">
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="relative">
                            <input :type="inputType" v-model="formData.password"
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="relative">
                            <input :type="inputTypeRepeat" v-model="formData.repeat_password"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 pr-10"
                                placeholder="Input repeat password" />
                            <button type="button" @click="togglePasswordVisibilityRepeat"
                                class="absolute inset-y-0 right-0 flex items-center pr-3 text-sm leading-5">
                                <UIcon v-if="showPassword" name="i-heroicons-eye-slash" class="text-gray-600" />
                                <UIcon v-else name="i-heroicons-eye" class="text-gray-600" />
                            </button>
                    ...
                    <div class="col-span-1">
                        <label for="nama_lengkap" class="block mb-2 text-sm font-medium text-gray-900">Nama
                            Panggilan</label>
                        <input type="text" id="nama_lengkap" v-model="formData.nama_panggilan"
                            placeholder="Input nama lengkap"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                        <span v-if="errors.nama_panggilan" class="text-red-600 text-sm">{{ errors.nama_panggilan
                            }}</span>
                    </div>
                    ...
                    <div class="col-span-1">
                        <label for="nama_lengkap" class="block mb-2 text-sm font-medium text-gray-900">Nama
                            Lengkap</label>
                        <input type="text" id="nama_lengkap" v-model="formData.nama_lengkap"
                            placeholder="Input nama lengkap"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                        <span v-if="errors.nama_lengkap" class="text-red-600 text-sm">{{ errors.nama_lengkap }}</span>
                    </div>
                    ...
                    <div class="col-span-1">
                        ...
                        <select v-model="selectedProvince" @change="fetchCitys" id="floating_company"
                            ...
                            <option v-for="province in formData.provinces" :key="province.id" :value="province.id">
                                {{ province.name }}
                            </option>
                        </select>
                    ...
                    <div class="col-span-1">
                        ...
                        <textarea id="alamat" v-model="formData.alamatPelanggan" placeholder="Input nama jalan"
                        ...
                        <span v-if="errors.alamatPelanggan" class="text-red-600 text-sm">{{ errors.alamatPelanggan
                            }}</span>
                    </div>

                    <div class="col-span-1 mt-2">
                        <label for="telepon" class="block mb-2 text-sm font-medium text-gray-900">Telepon</label>
                        <input type="tel" id="telepon" v-model="formData.telepon" placeholder="Input nama telepon"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                        <span v-if="errors.telepon" class="text-red-600 text-sm">{{ errors.telepon }}</span>
                    </div>
                </div>
